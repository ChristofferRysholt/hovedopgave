...
class Article
{
  ...
  */
   ...
   */
   ...
   public function storeUploadedImage($image) {
     if ($image['error'] == UPLOAD_ERR_OK) {
       ...
       if (is_null($this->id)) trigger_error("Article::storeUploadedImage(): Attempt to upload an image for an Article object thata does not have its ID property set.", E_USER_ERROR);
   ...
   */ 
   ...
   /**
    * Returns the relative path to the article's fulle-size or thumbnail image
    *
  ...
  */
  ...
  public static function getList($numRows=1000000, $categoryId=null) {
    ...
    $sql = "SELECT SQL_CALC_FOUND_ROWS *, UNIX_TIMESTAMP(publicationDate) AS publicationDate FROM articles $categoryClause ORDER BY publicationDate DESC LIMIT :numRows";
    $st = $conn->prepare($sql);
